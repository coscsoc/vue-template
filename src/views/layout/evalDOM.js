module.exports = function evalDOM() {
  ...
  if (agrsArr.length !== 1 || getArgtype(agrs) !== 'object') {
    //page.evaluate的arg都是string, 需要按照type生成对应类型
    //function:eval(fn), object:Json.parse(obj)
    agrs = parseAgrs([...agrsArr]);  //{init:fn,includeElement:fn...}
  }
  ...
  //<style>._{position:fixed;z-index:999;}.__{top:0%;left:0%;width:100%;}</style>
  ...
  */
  ...
  //有rgba 且 没有 border | box-shadow 且 width(0, 0.95) height(0, 0.3)
  //自定义块
  function isCustomCardBlock(node) {
    const bgStyle = getStyle(node, 'background');
    const bgColorReg = /rgba\([\s\S]+?0\)/ig;//rgba(xx,xx,x,0)
    const bdReg = /(0px)|(none)/;// 0px | none
      ...
      function deepFindNode(nodes) {
        if (nodes.length) {
          for(let i = 0; i < nodes.length; i++) {
            ...
            let node = nodes[i];
            //跳过display:none,visibility:hidden,opacity:0的隐藏节点, 配置中返回false的includeElement定制节点
            ...
            if ((includeElement(ELEMENTS, node) || //ELEMENTS中的节点
              backgroundHasurl || //背景图片
              //nodeType:3 文本节点 && textContent不为空 || 文本+子文本 || 自定义块 ||不在 自定义顶部header中
